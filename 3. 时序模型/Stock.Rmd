...
# 使用ggplot2绘制柱状图
ggplot(grouped_data, aes(x = Company, y = StdDev,fill = Company)) +
...
for (company in company_list) {
  ...
  if (length(test_set) > 0) {
    fit <- auto.arima(train_set, stepwise = FALSE, approximation = FALSE)   #判断Arima模型的参数值
    best_pdq_params[[company]] <- arimaorder(fit)
  } else {
    best_pdq_params[[company]] <- c(NA, NA, NA)
  }
  print(best_pdq_params[[company]])
}
...
forecast_period <- 20
history_period <- 40  # 设置历史数据长度为40天
