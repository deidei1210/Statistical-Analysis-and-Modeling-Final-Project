...
# 假设 stock_data 是您已经加载的数据框，它至少有 'Company' 和 'Close.Last' 列
company_list <- unique(stock_data$Company)
company_forecasts_10day <- list()
...
for (company in company_list) {
  ...
  # 存储模型和预测结果
  company_forecasts_10day[[company]] <- list(
    Model = fit,
    Forecast = forecast_result
  )
}
...
# 打印每个公司未来10天的预测结果
for (company in company_list) {
  forecast_info <- company_forecasts_10day[[company]]
  cat("公司:", company, "\n")
  cat("模型信息:\n")
  print(forecast_info$Model)
  cat("未来10天的预测结果:\n")
...
#思路：可以先带训练集使用auto.arima获得最佳参数，然后再直接不带训练集进行预测
...
forecast_period <- 20
history_period <- 20  # 设置历史数据长度为20天
...
for (company in company_list) {
  ...
  forecasted_price <- forecasted_diff
  ...
  p <- ggplot(forecast_data, aes(x = Date, y = Price, color = Type)) +
    geom_line() +
    # 添加连接历史数据最后一天和预测数据第一天的线段
    geom_segment(data = data.frame(
      x = last_observation, 
      y = last_historical_price, 
      xend = future_dates[2], 
      yend = forecasted_price[2]), 
...
for (company in company_list) {
  ...
  # 选取最近的历史数据，限制为20天
  historical_data <- subset(stock_data, Company == company & Date > (last_observation - history_period), select = c("Date", "Close.Last"))
  ...
  p <- ggplot(forecast_data, aes(x = Date, y = Price, color = Type)) +
    ...
    # 添加连接历史数据最后一天和预测数据第一天的线段
    geom_segment(data = data.frame(
      x = max(historical_data$Date), 
      y = last_historical_price, 
      xend = future_dates[2], 
      yend = forecasted_prices[2]), 
      aes(x = x, y = y, xend = xend, yend = yend), 
      color = "blue") +
    labs(title = paste("Stock Price Forecast for", company),
         x = "Date",
